...
class UTAEModule(nn.Module):
    def __init__(
        ...
        """
        super(UTAEModule, self).__init__()
...
class UTAE(EncoderDecoder):
    def __init__(self,
        ...
        self.model = UTAEModule(
                            input_size,
                            encoder_widths=[64, 64, 64, 128],
                            decoder_widths=[32, 32, 64, 128],
                            out_conv=[32, self.num_discrete_labels],
                            str_conv_k=4,
                            str_conv_s=2,
                            str_conv_p=1,
                            agg_mode="att_group",
    ...
    def forward(self, x):
        ...
        x = self.model(x, batch_positions=batch_positions)
        out = self.softmax(x)
        return out
